---
title: "A Governance Framework for AI in International Development"
date: 2026-06-15
pillar: "AI Governance"
dek: "A practical framework for aid organisations adopting AI without abandoning accountability."
...
For work that is sensitive, confidential and often culturally grounded, those risks are concrete rather than hypothetical:
...
Five components, each doing distinct work.

**An AI operating model.** AI is defined as scoped "agent" roles with explicit responsibilities, decision rights and escalation pathways, sitting under a human supervisory layer. Governing AI by *role* rather than by *tool* is what keeps the framework stable while the tools underneath it churn.
...
**A capability and training roadmap.** Guidance, playbooks and a path to embed the framework as lived practice, because a governance system that is written once and shelved governs nothing.

## Two hard rules

Underneath the components sit two non-negotiables that make the operating model legible to everyone using it:

> AI proposes, the principal decides. Nothing is released externally without human approval.

Most failures of AI governance are not exotic. They are an output that went out the door without a person standing behind it. Two rules close that door.

## The method: principles first, operating reality second
...
The components here — an operating model with decision rights and escalation, a use policy and guardrails, use-case verification and assurance, safeguarding controls, and capability building — are exactly the components of an enterprise AI governance capability. Only the scale differs. The design discipline, the governance logic and the stakeholder approach transfer directly to standing one up across a larger organisation.

That is the argument for practitioner-built governance. A framework written by people who use AI on sensitive work every day is grounded in the actual failure modes, not the ones a policy template imagines.

## The position

The part of the development sector that handles the most sensitive data — community testimony, beneficiary records, culturally held knowledge — is the part that can least afford to run AI ungoverned. The instinct to treat governance as a brake on adoption has it backwards. Governance is not the cost of using AI in development. It is the precondition for using it at all without quietly trading away the accountability the work depends on.

*Written by Aydon Edwards, Principal, Moala Global. Moala Global advises on gender, political economy and the governance of AI in international development.*
